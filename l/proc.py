...
while True:
    ...
    if i >= N:
        ...
        maskb = sobel_abs > 0.045
        # mask = ((maskb)*255).astype("uint8")
        ...
        # m2 = mask
        # m2 = cv2.morphologyEx(m2, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)),iterations = 1)
        # # sobel_abs = m2 * (sobel_abs)
        # m2 = cv2.dilate(m2,cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(11,11)),iterations = 1)
        ...
        cv2.circle(img, _nt(track_point), 10, (255, 0, 255), 4)
        cv2.circle(img, _nt(blink_point), 10, (255, 255, 0), 4)
